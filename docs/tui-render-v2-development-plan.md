...
- 发布必须验证并发布同一个 tarball，固定 shell 流程为：`pnpm compile`; `outDir="${RUNNER_TEMP:-$(node -p "require('os').tmpdir()")}/tui-v2"`; `mkdir -p "$outDir"`; `packJson="$outDir/pack.json"`; `rollbackManifest="$outDir/rollback-manifest.json"`; `npm pack --ignore-scripts --json --pack-destination "$outDir" --foreground-scripts=false > "$packJson"`; `tgz=$(cd "$outDir" && node -e "const r=JSON.parse(require('fs').readFileSync(process.argv[1])); const x=Array.isArray(r)?r[0]:Object.values(r)[0]; process.stdout.write(require('path').resolve(x.filename))" "$packJson")`; `sha=$(sha256sum "$tgz" | cut -d' ' -f1)`; `node scripts/verify-package.mjs < "$packJson"`（保留当前 stdin-only package surface 检查）；`node scripts/verify-tui-v2-tarball.mjs --tarball "$tgz" --sha256 "$sha" --pack-json "$packJson" --rollback-manifest "$rollbackManifest"`（检查 exact tgz 内容、file manifest、license/NOTICE、exports、依赖和 rollback manifest，并生成 `verified-tarball.json`）；最后 `npm publish "$tgz" --ignore-scripts`。`.github/workflows/publish.yml` 必须消费 `verified-tarball.json` 中的同一绝对路径/hash，不得在验证后执行普通 `npm publish` 或触发第二次 lifecycle/compile；根包不定义 `prepare`，第一步只允许显式运行一次 `pnpm compile`，发布步骤必须带 `--ignore-scripts` 并对 exact `.tgz` 做 registry publish response/hash 记录。`RUNNER_TEMP` 未设置时 wrapper 使用 `os.tmpdir()/tui-v2`，不能把占位符传给 shell。
...
- WP-08g 落地时锁定终端能力与高级协议边界：`TerminalCapabilitySnapshot@v1` 由 profile、allowlisted environment presence bits 和 generation/token-fenced query evidence 归一化，snapshot/registry 深冻结且 stale refresh 不得回写；unknown/timeout/late response 只产生 conservative unsupported reason，不发送危险序列。mouse 保留完整 `MouseTrackingMode`（tracking 与 SGR/urxvt/X10 encoding 互斥复位），wheel 经过窄 controller 进入 scrolling，selection/search/overlay/cursor 缺失时明确降级；focus、resize、SIGWINCH/SIGCONT/repeated signal 和 cleanup 幂等。Kitty keyboard query/enable/disable 失败回退 legacy keys，失败与 late response 不阻塞 startup/stop。OSC52 仍只经 trusted `ansi.osc52Clipboard` 与 confirmed profile/policy gate，payload/bytes 只保留长度与 SHA-256；custom theme roles 深冻结并拒绝控制色值；profile 未确认 truecolor 时固定量化到 ANSI 256，i18n 只保证逻辑顺序与 profile width，不做 bidi shaping。`host-capabilities@v1` 及 `--check host-capabilities` 只证明 fake stream/profile；Windows/ConPTY、macOS/iTerm2、SSH/tmux/VS Code/JetBrains 与真实 PTY 保持 manual/deferred，不由 CI fixture 虚报。回滚方式：移除 capability detector、mouse controller、Kitty negotiator、host check/trace 与 WP-08g matrix rows，恢复 profile-only gates；不修改 vendor、goldens、依赖或 lockfile。

- WP-09a 落地时锁定离线 baseline 与最终 gate 的边界：`tools/tui-v2-baseline/` 只回放带 source commit/tree hash、文件 hash、MIT license hash 的冻结 artifact；`V1CaptureRenderer` 在 fake writer/clock/stdin/no-op adapter 下运行，side-effect ledger 的 stdout/stderr、live subscription、timer、command、session write 必须为零，唯一允许的写入是 fake writer，报告只保留脱敏 hash/坐标。tools 不得被 `src`、bootstrap、package exports 或 runtime tarball 反向引用。当前旧 React/Ink/Yoga 链仍由 WP-09b 删除，因此 `--check v2-only` 默认 staged：runtime boundary、artifact shape/hash 和最小 compare smoke 为硬门，旧链路与缺 rollback manifest 以 `deferredTo: WP-09b/WP-09c` 记录且不虚报最终通过；`--final` 或 `TUI_V2_FINAL=1` 将全部 deferred/mismatch 转为非零。rollback manifest 只在本项做 schema、exact version/tarball basename、SHA-256、signature、session schema、launcher timeout/retry 和 retention 的本地前置检查，不联网、不在进程内切 renderer；可执行 tarball/launcher/child-process drill 留 WP-09c。回滚方式：删除 tools tree、v2-only 分支和 compare artifact/check 后恢复 WP-09 前人工 baseline 记录，不修改 vendor、goldens、依赖或 lockfile。
- WP-09b 阶段1 锁定生产接线与 helper 边界：`src/dsh-adapter/plugin.ts` 只保留 DSH/Cordis/agent/channel/store、stderr guard、background update check、resume marker 与 domain teardown，生产 UI 通过单一 `createTuiV2App`/coordinator 启动，不保留 React/Chat/Ink mount、fallback 或双 run；approval/question/plugin-dialog stores、SceneV2 runtime、status/shortcut host 均经 factory 的 structural narrow seam 透传，coordinator 不依赖 dsh-adapter 的 host service 类型。`TerminalProfile` 生产 resolver 只消费 stdout/stderr TTY、有效尺寸、`TERM`/`COLORTERM`、platform 及 allowlisted `TMUX`/SSH/WT_SESSION/VSCODE presence bits；unknown host 保守为 unknown-conservative，自动模式可明确降级 inline，显式 fullscreen 在 alternate-screen 未被证明时返回 `unsupported-alternate-screen`，不静默改成 fullscreen 或生产测试 profile。`/exit`/`/quit`/`/q` 与 Ctrl+D 只调用 coordinator stop funnel；Ctrl+C/D 的二次确认、signal/stdin/error stop 统一等待 v2 stop barrier 后才写 resume marker、dispose root 或 update handoff，绝不由 input/commands 写 ANSI 或 `process.exit`。activity frame presets、SpinnerMode 与 cell-width/sanitize helper 位于 neutral `src/utils` 路径；external-editor utility 只保留 argv/path/cmd.exe 纯 helper，child execution 与 takeover 归 v2 capability/coordinator。旧 React/Ink 树、旧 repro 与 legacy external-editor UI script 暂保留到阶段3，不将其称为生产覆盖。回滚方式：恢复 plugin 的旧 mount 仅用于离线旧 UI 回归，不在同一进程与 v2 fallback；保留 stores/profile/exit/helper seam 的测试和阶段1静态边界。

- WP-09b2 阶段2 锁定 live smoke/benchmark/workflow 迁移与旧 helper 退役：公开 `pnpm smoke` 改为 `scripts/smoke-tui-v2.mjs`，只经 `createTuiV2App`/coordinator、注入 fake stdin/stdout/stderr、v2 controller-rig 与 `VirtualTerminal` 验证首帧、输入行、stop 和 terminal restore，禁止把真实 `process.stdout` 当终端写入；`bench:tui-v2` 改为 v2 coordinator startup/stop、stream frame latency 与 `VirtualTerminal` grid/hash 采样，保留身份字段、GC fail-fast 和 artifact schema，默认 fixture 不再依赖旧 React harness。删除 live-only `test/tui-v2/helpers/harness.tsx`、`test/tui-v2/helpers/lifecycle-child.tsx`、`scripts/compare-tui-v2-skeleton.ts` 与 `docs/tui-v2/baseline/compare-skeleton.json`；`lifecycle.test.ts` 改走 v2 bootstrap，不能以同名旧 helper 或“SKIP”伪造覆盖。`.github/workflows/ci.yml`/`publish.yml` 不再执行旧 React UI repro，UI 回归集中到 `test:tui-v2` 的 name pattern 与 `verify:tui-v2` 的 controllers/fullscreen/inline/host-capabilities/v2-only checks；仍保留 build/verify:build/package、v2 test/verify 和纯 domain gates（session index/cwd/titles/kinds、model route、plugin negotiation、external-editor argv/controller seam）。regression matrix 重扫 workflows/package 的 64 个入口并更新 listHash；旧 live CI 行在本阶段标记 `retired`/`blockDefault:false`，v2 smoke/benchmark 行标记 `verified`。`tools/tui-v2-baseline/` 仍是唯一可执行 offline baseline 边界，不移入 package、不由 live helper 依赖；manifest 保留历史 source file/hash/license/artifact provenance，source 文件因阶段2/3 删除时只进入 `missingSourceFiles`，`sourceMismatches`、artifact hash、artifact/source tree、MIT license hash 仍必须验证，禁止重建 current source。阶段2 不删除 `src/ink`、`src/components`、`src/screens`、`src/ui.ts`，阶段3 再完成旧 runtime 树清理；本决策不修改 `src/tui-v2/vendor/**`、`test/tui-v2/goldens/**`、依赖或 lockfile。回滚方式：恢复 workflow 中旧入口并恢复 smoke/bench 旧脚本与 helper（不与 v2 同进程双跑），或单独回退本阶段文档/matrix/入口变更；不得把 tools baseline 变成生产 fallback。
- WP-09b3 阶段3锁定 legacy-clean final policy：删除 `src/ink/**`、旧 `src/components/**`、`src/screens/**`、Yoga port、旧 `src/ui.ts`/React force shim、旧 only helpers 与所有旧 renderer repro/verify scripts；保留 `src/sessions/view.ts`、v2 components/terminal vendor、neutral `src/utils/**` 与 domain checks。`themePrefs` 只持久化安全 v2 registry id；`history.ts` 通过 bootstrap persistence seam seed/append v2 editor history，跨进程 load/append 语义不得丢失。`scripts/verify-tui-v2.ts --check v2-only` 的 strict scan 要求 source paths/switches/JSX/direct imports=0；baseline source files 可 missing，但 artifact/source-tree/license hashes 与 `reviewed-differences.json` ledger 必须 hash-pinned。WP-09a grid/cursor/modes mismatch 不用 allow-mismatches 掩盖：仅登记明确 P2 visual difference，P0/P1 semantic assertions 必须由 versioned v2 tests/checks 覆盖。最终 `--final` 只允许因 WP-09c2 rollback/publish artifact 缺失非零；本阶段不实现 rollback manifest、publish 或 soak。回滚方式：单独恢复阶段3删除/manifest/docs/matrix 变更，不在生产恢复 renderer 双跑。
- WP-09c1 锁定 bounded soak、ownership 与 CI integration 的九项决策。其一，公开入口逐字为 `node --expose-gc --import tsx/esm scripts/soak-tui-v2.ts`，启动即 fail-fast 检查 `global.gc`；默认 `unknown-conservative`、120x40、seed 1、仓库内 fake duplex + `VirtualTerminal`，`--require-pty` 才动态加载 exact `node-pty@1.1.0` 并以 PTY/ConPTY 子进程运行同一 v2 coordinator，任何 native/host capability 失败都写 `pty-unavailable` artifact 并非零，绝不退回 fake 或 skipped。其二，soak 以四个不计入门槛、与正式轮换同构的完整 20k settled warm-up window 预热 V8/JIT/native stream page（数量显式写入 artifact，不隐藏为正式覆盖）；正式 event gate 必须再完成至少 100k settled 和五个完整 20k window，duration gate 必须实际达到 10 分钟；更短 `--minutes` 只验证 schema/resource/latency presence，固定 `coverageMode: smoke`、`gateEligible:false`，短真实 PTY 同样不能冒充 8/24 小时。其三，每个正式 window 在 scheduler/writer idle、latency pending task 全部 settled 且跨一个 event-loop turn 后，按 `GC → event-loop turn → GC` 采样并在 artifact 记录固定 settling protocol id，再记录 midpoint、GC 前/后 heapUsed 和 GC 后 RSS；least-squares slope 固定换算 MiB/10k、常量序列 `R²=0`、monotonic ratio 为相邻严格增加次数/(n-1)。heap/RSS 每个窗口（不只 final）相对首窗必须分别 `<=1.25/1.5`，heap/RSS slope 都 `<=1 MiB/10k`，且两种资源各自同时满足 monotonic `<0.9` **AND** `R²<0.8`；缺窗、NaN/Infinity 或任一布尔失败均 fail，不存在 OR waiver。其四，artifact 只含 identity、计数、hash、latency、queue/resource/window 标量；完整记录 runner/CPU/cores/RAM/container image+digest/OS/kernel/Node/npm/pnpm/HEAD+dirty/lock hash/argv/profile/fixture/seed/time/terminal type，不保存 prompt、raw terminal bytes、环境或 credential。其五，`TerminalWriter.stats()` 与 scheduler/coordinator diagnostics 暴露当前/峰值 frame queue、pending bytes、in-flight/backpressure、generation、timer/listener/frame queue；生产 scheduler 保证 frame queue `<=2`、writer pending `<=8 MiB`，stop 后 queue/timer/listener 不增长。其六，ownership gate 从 bin、公开 index、adapter plugin、v2 bootstrap roots 解析 runtime dependency closure并用 TypeScript AST 扫 stdout/stderr/console、terminal stream、guard、external child 和 control literals；机器块同时冻结 owner/lifecycle/backpressure/cleanup/generation/queue 与全命中 hash。生产 editor/input 分别使用收窄 `pi-editor.ts`/`pi-input.ts`，全量 `pi.ts`、`ProcessTerminal`、pi main/alt screen 不得生产可达；唯一物理 frame/control/query lane 是 `TerminalWriter`，live drill 验证 barrier、stale generation、query、backpressure 和 cleanup。其七，PR Node 22.19/24 matrix 保留 compile/test/check/package 并逐字运行 10 分钟 fake soak；build job只跑短 smoke。nightly/release 的 Ubuntu/macOS/Windows 必须 `--require-pty`，因 hosted 单 job 上限，nightly 明确链为 `2×240m=480m`，release 明确链为 `5×288m=1440m`；每段是独立进程/real-PTY artifact，continuation verifier 校验 run/host/Node/profile/seed、前链 SHA-256、段号和总时长，aggregate 不声称单进程 24h。其八，`--check ci-integration` 解析全部 workflow YAML、记录文件 hash/HEAD，静态验证双 Node、wrapper/reporter、全部 checks、bench/soak/upload/package/host chain，并运行非递归 ownership、短 fake soak、bench、package dry-run probes，记录实际 argv/exit/artifact path+hash；不能递归启动自身。其九，exact verified-tarball publish 明确留给 WP-09c2：ci-integration staged 以唯一 `deferredTo: WP-09c2` 通过，`--final` 只因此非零；本项不修改普通 publish、不实现 rollback manifest/tarball verifier。回滚方式：移除 soak/stats/node-pty/host workflows、ownership/ci checks 与窄 pi facades，恢复 WP-09b3 staged gates；不得恢复旧 renderer 或双 writer。

- WP-09c2 锁定 rollback manifest、exact tarball 与 verified publish 的发布边界：`create-tui-v2-rollback-manifest.mjs` 不提供 production 默认值，registry/package/exact previous version/tarball SHA-256/signature ref/session schema/launcher timeout-retries/retention 均由 release runner 显式输入；`verify-tui-v2-tarball.mjs` 只消费一次 `npm pack --ignore-scripts --json --foreground-scripts=false` 产生的 exact tgz/pack JSON；根包删除 `prepare`，compile 只能由 release workflow 的单次显式步骤执行（npm 10 即使带 `--ignore-scripts` 仍可能触发生命周期并污染 JSON），先拒绝 path traversal、重复 entry、link/device/FIFO，再安全提取并核对 package identity、file manifest/hash、main/types/bin/exports、旧 React/Yoga/第二 renderer、root LICENSE 与 pi fork LICENSE/NOTICE/PATCH-LEDGER/VENDOR-MANIFEST。`verified-tarball.json` 原子记录 absolute tgz/hash、pack/file/export/dependency/rollback/toolchain identity，不保存 signature ref 或环境秘密；publish workflow 只下载由显式 run-id 指定的上一 verified release artifact，compile/pack 各一次，final ci-integration/v2-only 与 child cleanup drill 通过后才从 verified artifact 读取同一路径/hash执行 `npm publish "$tgz" --ignore-scripts --access public --provenance --json`，并上传 response/hash record。本地 fixture 明确使用 current tgz + fixture signature 并标为 `fixture`/`unsupported-by-host`，不冒充 production previous release；当前进程只 cleanup 后非零退出，external launcher dry-run 不联网且不在进程内切 renderer。回滚方式：删除 WP-09c2 scripts/tests/release workflow contract，恢复 final gate 的 deferred；不得恢复普通 `npm publish` 或同进程 renderer fallback。
