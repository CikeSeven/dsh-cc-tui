...
export function ScrollbarGutter({
  ...
  // top follows the last position actually painted over the scroll range.
  const thumbH = Math.max(2, Math.round((viewport * viewport) / content))
  const trackH = Math.max(1, viewport - thumbH)
  const paintedTop = handle.getRenderScrollTop?.() ?? handle.getScrollTop()
  const thumbTop = Math.round((paintedTop / Math.max(1, maxScroll)) * trackH)
  const thumbBottom = Math.min(viewport, thumbTop + thumbH)

  // Clicking the track maps the clicked row's position on the track back
  // to a raw scroll target and scrolls that content position to the viewport top.
