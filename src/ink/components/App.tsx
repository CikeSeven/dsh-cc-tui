...
function processKeysInBatch(
	...
	for (let i = 0; i < items.length; i++) {
		...
		if (sequence === FOCUS_OUT) {
			...
			// Safe boundary: drain any deferred alt-screen re-entry or blocked
			// probe now that no button can be held and no candidate is in
			// flight (focus-out is a terminal protocol event, not SGR).
...
export function handleMouseEvent(app: App, m: ParsedMouse): void {
	...
	if (m.action === "press") {
		if ((m.button & 0x20) !== 0 && baseButton === 3) {
			...
			app.finishDragSession();
			// Safe boundary: drain any deferred alt-screen re-entry or blocked
			// probe now that no button can be held.
			app.props.onReleaseTail?.();
	...
	try {
		...
		// Release tail: the gesture latch cleared at the START of this release,
		// but a deferred alt-screen re-entry (or a probe blocked by the gesture)
		// must wait until NOW — all drag/click/selection handling above has
		// finished reading frontFrame (cellIsBlank, getHyperlinkAt), and
		// reenterAltScreen resets those frames synchronously. The finally
		// covers every return path: started dragend, dormant click replay,
		// ordinary click, and the inert middle/right release.
		app.props.onReleaseTail?.();
	}
}
