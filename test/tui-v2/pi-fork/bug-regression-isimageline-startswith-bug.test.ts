/**
 * pi fork: ported from pi packages/tui/test/bug-regression-isimageline-startswith-bug.test.ts
...
describe("pi fork: Bug regression: isImageLine() crash with image escape sequences", () => {
	describe("Bug scenario: Terminal without image support", () => {
		...
		it("new implementation returns true correctly", async () => {
			const { isImageLine } = await import("../../../src/tui-v2/vendor/pi-tui/src/terminal-image.js");
