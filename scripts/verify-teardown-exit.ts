 ...
 * recompose disposes every cordis entry once, which can settle the application
 * lifecycle just like a user exit. Before the fix both paths ran the full
 * leave sequence, so the process exited 0 with no error mid-recompose:
 * "flash back to bash, no error message".
 *
